...
sys.path.append(r"/home/chen/desire_10086/flat_ped_sim/src/arena-rosnav/arena_navigation/arena_local_planner/learning_based/arena_local_planner_drl/tools")
...
def d86_train():
    ...
    # args.visible_gpu = '1'  # 使用的gpu编号
    args.worker_num = 4  # 每个learner下有两个worker来采集轨迹
    ...
    args.learning_rate = 3e-4  # 3e-5 1e-4
    #####
    ...
    args.gamma = 0.99  # 0.985  # 未来回报折扣因子 越小越不看重未来收益(短视) 大看的远 在DRL中agent“看得远”表面上指的是向前考虑的步数多，实质上是指agent向前考虑的系统动态演化跨度大
    ...
    args.target_step = 16000 # args.env_max_step * 8


    # args.env.max_step * 8  2 ** 12  # 采集到不多不少target_step步的经验就会对策略进行一次的update

    args.break_step = int(6e6)  # 当总的经验池历史收集步数 综合达到 这个步数 结束训练
