...
from agent import *
from env import PreprocessEnv, build_env,get_gym_env_info
...
def demo_continuous_action_on_policy():
    ...
    # args.visible_gpu = '0,1'  # 使用的gpu编号
    args.visible_gpu = '0,1'  # 使用的gpu编号
    args.worker_num = 2  # 每个learner下有两个worker来采集轨迹
    args.thread_num = 12  # 114服务器是12核的
    args.env_num = 1  # 每个worker下只有一个env 每个env的random_seed都不一样
    args.which_env = 1 # 0 空环境 1 静态障碍 2 动态障碍
    # 参数的设定 网络的结构 网络的加载
    ...
    args.learning_rate = 5e-5  # 3e-5 1e-4
    #####
    ...
    # 360 sensor size + 4 self state size 环境单步返回的状态的维度为360个雷达信息+4自身运动状态信息
    args.env_sigle_state_dim = 360 + 2 + 2  # 加上了roll 和 pitch 作为状态
    args.env_state_stack_times = 4
    args.env_state_dim = 3 # + 360
    args.env_get_goal_dist_accuracy = 0.25  # 位置精度-->判断是否到达目标的距离
    args.env_target_return = -200
    # 训练前期400 
    args.env_max_step = 200 # 1/(1-gama)  gama=0.98 maxstep = 500 gama=0.99 maxstep = 1000
    args.env_action_dim = 1
    args.env_action_max = [3.0, 4.0]  # v,w m/s rad/s
    ...
    args.gamma = 0.97  # 0.985  # 未来回报折扣因子 越小越不看重未来收益(短视) 大看的远 在DRL中agent“看得远”表面上指的是向前考虑的步数多，实质上是指agent向前考虑的系统动态演化跨度大
    ...
    args.agent.ratio_clip = 0.22 # 0.1-0.3 Cliprange越小训练越稳定，越大越节省数据。一般在训练早期取较小的值，比如0.2，保证训练平稳进行；到训练后期可以适当放大，因为此时policy已经足够优秀，所采集数据中正样本比例非常高，可以放心利用。
    ####
    # args.net_dim = 2 ** 8  # actor和critic全连接中间层的维度
    args.net_dim = 2 ** 7  # actor和critic全连接中间层的维度
    args.batch_size =  2 ** 8 # args.net_dim * 4  # 单次策略优化时 从经验池中sample batch_size步的数据进行 网络的更新
    ...
    args.max_memo = args.target_step  # 经验池大小 = max_memo + max_step
    ####
    args.if_allow_break = True  # 不允许提前终止
    ...
    if if_train_pendulum:
        ...
        (args.env.env_name, args.env.state_dim, args.env.action_dim, args.env.action_max, args.env.max_step,
         args.env.if_discrete
        ) = 'Pendulum-v0',3,1,2.,200,False# = get_gym_env_info(args.env, True)
    ...
    if if_train_lunar_lander:
        ...
        (args.env.env_name, args.env.state_dim, args.env.action_dim, args.env.action_max, args.env.max_step,
         args.env.if_discrete
         ) = 'LunarLanderContinuous-v2',8,2,1.,1000,False# = get_gym_env_info(args.env, True)
        args.gamma = 0.99
        args.env.target_return = 200
    ...
    if if_train_bipedal_walker:
        ...
        (args.env.env_name, args.env.state_dim, args.env.action_dim, args.env.action_max, args.env.max_step,
         args.env.if_discrete
         ) = 'BipedalWalker-v3',24,4,1.,1600,False# = get_gym_env_info(args.env, True)
