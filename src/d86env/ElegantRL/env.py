...
class PreprocessEnv(gym.Wrapper):  # environment wrapper
    def __init__(self, env, if_print=True, if_norm=False, data_type=np.float32):
...
def build_gazebo_env(args, env_id=0):
    if args.which_env == 0:
        from gazebo_env_empty_30m import envmodel
    elif args.which_env == 1:
        from gazebo_env_static_obstacle_30m import envmodel
    elif args.which_env == 2:
        from gazebo_env_dynamic_obstacle_30m import envmodel
    # 因为在envmodel有rospy init 所以一个进程只能调用一次这个build_env函数
    train_env = envmodel(args, env_id)
    return PreprocessEnv(env=train_env)
...
def build_arena_env(args, env_id=0):
    
    env_ =  make_env(True, env_id, 
                            params=args.arena_param, 
                            PATHS=args.arena_path,
                            train=(env_id != 0),
                            which = args.which_env)
    env = env_()
    env._max_steps_per_episode = args.env_max_step
    env = PreprocessEnv(env=env)
    env.env_name = env.env.ns
    env.state_dim = args.env_state_dim
    env.action_dim = args.env_action_dim
    env.action_max = args.env_action_max
    env.max_step = args.env_max_step
    env.if_discrete = False
    env.target_return = args.env_target_return
