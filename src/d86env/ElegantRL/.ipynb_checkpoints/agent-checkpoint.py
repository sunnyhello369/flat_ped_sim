...
class AgentBase:
    ...
    def init(self, net_dim, state_dim, action_dim, learning_rate=1e-4,
        ...
        self.cri_optim = torch.optim.AdamW(self.cri.parameters(), learning_rate)
        self.act_optim = torch.optim.AdamW(self.act.parameters(), learning_rate) if self.ClassAct else self.cri
    ...
    def optim_update(optimizer, objective):
        optimizer.zero_grad()
        objective.backward()
        # clip_grad_norm_(parameters = optimizer.param_groups[0]["params"],max_norm=0.5)
        clip_grad_norm_(parameters = optimizer.param_groups[0]["params"],max_norm=1.0)
        # clip_grad_norm_(parameters = optimizer.param_groups[0]["params"],max_norm=3.0)

        # max_norm SGD/adamW 3.0 adam 0.5
        optimizer.step()
...
class AgentPPO(AgentBase):
    ...
    def explore_one_env(self, env, target_step):
        ...
        for i in range(target_step):
            ...
            # next_state, reward, done, _ = env.step(AgentPPO.sigmoid(action))
            a = []
            for act in action:
                a.append(AgentPPO.sigmoid(act))
    ...
    def explore_one_env2(self, env, target_step):
        trajectory_temp = list()
        state = self.states[0]
        last_done = 0
        last_done_ = 0
        ...
        for i in range(target_step):
            ...
            if last_done != i-1 and last_done != 0:
                self.last_sars[1] = reward
                self.last_sars[2] = done
                trajectory_temp.append(self.last_sars)

            self.last_sars = [state, reward, done, action, noise]

            if done:
                state = env.reset()
                last_done = i
                last_done_ = len(trajectory_temp) - 1
            else:
                state = next_state
        ...
        '''splice list'''
        trajectory_list = self.trajectory_list[0] + trajectory_temp[:last_done_ + 1]
        self.trajectory_list[0] = trajectory_temp[last_done_:]

        return trajectory_list

    def explore_vec_env(self, env, target_step):  # 一个worker如何在多个env下探索 传入的env是VecENV类型的
        env_num = len(self.trajectory_list)
        states = self.states

        trajectory_temps = [list() for _ in range(env_num)]
        last_done_list = [0 for _ in range(env_num)]
        for i in range(target_step):  # 在每个env中都走target_step步
            actions, noises = self.select_actions(states)  # 还是同用一个网络 在不同的环境下 去采集轨迹 可以更快地采集轨迹
            s_r_d_list = env.step(np.tanh(actions))  # 调的是VectorEnv主进程的step
    ...
    def update_net(self, buffer, batch_size, repeat_times, soft_update_tau):
        with torch.no_grad():
            ...
            bs = 2 ** 9  # set a smaller 'BatchSize' when out of GPU memory.
        ...
        obj_critic = None
        obj_actor = None
        obj_entropy_log = None
        obj_surrogate_log = None
        ...
        # for _ in range(3):
            ...
            obj_entropy_log = obj_entropy #用于打印显示
            obj_surrogate_log = obj_surrogate
        ...
        print("*******************************************")
        print("after")
        print(self.cri_optim.param_groups[0]['params'][0].grad[0])
        print(self.cri_optim.param_groups[-1]['params'][-1].grad[0])
        # 检查梯度是否出现了梯度消失或梯度爆炸
        grad_cri_list = []
        for name, param in self.cri.named_parameters():
            if param.requires_grad and 'bias' not in name and param.grad is not None:
                grad_max = torch.abs(param.grad.data).max()
                grad_min = torch.abs(param.grad.data).min()
                grad_cri_list.append(param.grad.detach().flatten())
                if grad_max == float('inf') or grad_min == float('-inf') or torch.isnan(grad_max) or torch.isnan(grad_min):
                    print(grad_max,grad_min)
                print(f'{name} cri gradient: max={grad_max}, min={grad_min}')
                print(f'{name} cri: Mean - {param.grad.mean().item()}, Std - {param.grad.std().item()}')
            elif param.grad is None:
                print("cri gradient have none")
        grad_tensor = torch.cat(grad_cri_list)
        print("cri mean:",grad_tensor.mean(),"cri std:",grad_tensor.var())

        print(self.act_optim.param_groups[0]['params'][0].grad[0])
        print(self.act_optim.param_groups[-1]['params'][-1].grad[0])
        grad_act_list = []
        for name, param in self.act.named_parameters():
            if param.requires_grad and 'bias' not in name and param.grad is not None:
                grad_max = torch.abs(param.grad.data).max()
                grad_min = torch.abs(param.grad.data).min()
                grad_act_list.append(param.grad.detach().flatten())
                if grad_max == float('inf') or grad_min == float('-inf') or torch.isnan(grad_max) or torch.isnan(grad_min):
                    print(grad_max,grad_min)
                print(f'{name} act gradient: max={grad_max}, min={grad_min}')
                print(f'{name} act: Mean - {param.grad.mean().item()}, Std - {param.grad.std().item()}')
            elif param.grad is None:
                print("act gradient have none")
        grad_tensor = torch.cat(grad_act_list)
        print("act mean:",grad_tensor.mean(),"act std:",grad_tensor.var())

        a_std_log = getattr(self.act, 'a_std_log', torch.zeros(1))
        return obj_critic.item(), obj_actor.item(), a_std_log.mean().item(),obj_entropy_log.item(),obj_surrogate_log.item()  # logging_tuple
