...
void PlanManager::init(ros::NodeHandle &nh)
{
  ...
  n.param("/use_d86_env", d86_env_, false);
  ...
  // DEBUG
  /* callback */
  exec_timer_ = nh.createTimer(ros::Duration(0.01), &PlanManager::execFSMCallback, this); // 100hz
...
void PlanManager::execFSMCallback(const ros::TimerEvent &e)
{
  ...
  case GEN_NEW_GLOBAL:
  {
    ...
    { 
      ...
      bool global_plan_success = planner_collector_->generate_global_plan(*start_state_, *end_state_); // hybrid A*无优化
  ...
  case EXEC_LOCAL: // 训练local执行期间无动作
  {
    if (mode_ == TRAIN)
    {
