...
void InterPlanner::goalCallback(const geometry_msgs::PoseStampedPtr& msg){
  ...
  // // start pt
  start_pt_(0) = msg->pose.orientation.x;
  start_pt_(1) = msg->pose.orientation.y;
  start_vel_.setZero();
  // start_pt_  =  odom_pos_;
  ...
  if (VIS){
    ...
    p.header=msg->header;     
    p.pose=msg->pose;
    std::cout << start_pt_(0)<<","<<start_pt_(1) << std::endl;
  ...
  // first search
  status = global_planner_kino_astar_->search(start_pt_, Eigen::Vector2d::Zero(), Eigen::Vector2d::Zero(), end_pt_, Eigen::Vector2d::Zero(), true);
  ...
  if (status == KinodynamicAstar::NO_PATH) {
    ...
    status = global_planner_kino_astar_->search(start_pt_, Eigen::Vector2d::Zero(), Eigen::Vector2d::Zero(), end_pt_, Eigen::Vector2d::Zero(), false);
...
bool InterPlanner::makeGlobalPlan(arena_intermediate_planner::Global_Kino_MakePlan::Request  &req, arena_intermediate_planner::Global_Kino_MakePlan::Response &res){
  ...
  std::cout<<"world start "<<start_pos(0)<<","<<start_pos(1)<<"  world end "<<end_pos(0)<<","<<end_pos(1)<<std::endl;
