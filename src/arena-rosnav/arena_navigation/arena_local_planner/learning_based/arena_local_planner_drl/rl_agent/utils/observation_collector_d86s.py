...
class ObservationCollector_d86:
    def __init__(
        ...
        self.opt_ctrl_points_num = opt_ctrl_points_num
        self.max_vertex_num = 60
        self.sigle_state_dim = self.max_vertex_num*2 + 2 + 1 + 1 + self.opt_ctrl_points_num*(2 + 2) + 1
        # self.sigle_state_dim = num_lidar_beams + 2 + 1 + 1 + self.opt_ctrl_points_num*(2 + 2) + 1
        ...
        self.scan_que = np.zeros((self.stack_times,2*self.max_vertex_num), dtype=float)
        self.state_que = np.zeros((self.stack_times,4 + 1 + self.opt_ctrl_points_num*(2 + 2)), dtype=float)
        ...
        self._clock = Clock()
        self.msg_galaxy2d = Galaxy2D()
        self.msg_galaxy2d.success.data = False
        # self.g2d_cal_success = False
        # # self.scans = np.zeros(self.laser_num_beams, dtype=float)
        # self.scans = [0. for i in range(self.laser_num_beams)]
        # self.g2d_convex_vertex = []
        # self.g2d_polar_convex = [0. for i in range(self.laser_num_beams)]
        # self.g2d_polar_convex_theta = [i*2*np.pi/self.laser_num_beams for i in range(self.laser_num_beams)]


        self._scan_xy_robot_frame = []
        self.state = None
        self._robot_pose = Pose2D()
        self._last_robot_pose = Pose2D()
    ...
    def get_observations3(self, last_polar_action_info,last_world_action_info):
        # apply action time horizon
        # if self._is_train_mode:
        #     # 向前仿真到需要采取下个动作的周期
        #     self.call_service_takeSimStep(self._action_frequency)
        ...
        self.wait_for_sensor_info()
        if last_polar_action_info is not None:
            self.last_done = False
        
        # if self.scans is None or len(self.scans) != self.laser_num_beams:
        #     self.scans = np.zeros(self.laser_num_beams, dtype=float)
        ...
        msg_galaxy2d = deepcopy(self.msg_galaxy2d)
        ...
        #     base_frame_x = scans[i]*np.cos(base_frame_theta) + self.laser_origin_in_base[0]
        #     base_frame_y = scans[i]*np.sin(base_frame_theta) + self.laser_origin_in_base[1]
        #     scans_xy.append((scans[i]*np.cos(base_frame_theta),scans[i]*np.sin(base_frame_theta)))
        # convex_vertex_now,polar_convex_now,polar_convex_now_theta = galaxy_xyin_360out(scans_xy,radius = self.lidar_range)
        # convex_vertex_now,polar_convex_now,polar_convex_now_theta = None,None,None
        # rho, theta = ObservationCollector_d86._get_goal_pose_in_robot_frame(
        #     self.goal, self._robot_pose,self._robot_vel
        # )
        rho, dtheta, prho, pdtheta = self._get_goal_pose_in_robot_frame(
            self.goal, self._robot_pose,self._robot_vel,last_world_action_info
        )

        
        g2d_cal_success = msg_galaxy2d.success.data
        # self.scans = np.array(msg_galaxy2d.scans)
        ...
        n_scan,n_polar_convex,ndtheta,nrho,npdtheta,nprho,nrvx,nrvy\
            ,n_polar_action_rad,n_polar_action_dist,n_opt_polar_action_rad,n_opt_polar_action_dist \
            = self.normalize_obs(g2d_cal_success,scans,g2d_polar_convex,dtheta,rho,pdtheta,prho,self._robot_vel.linear.x,self._robot_vel.linear.y,last_polar_action_info)

        # obs_dict用于记录统计并且是reward计算需要的信息，mergerd用于环境返回
        # 测试一下看回调传回的速度是在机器人坐标系下的
        if g2d_cal_success and len(g2d_polar_convex) == self.laser_num_beams:
            process_scan = n_polar_convex
        else:
            g2d_cal_success = False
            process_scan = n_scan
            g2d_convex_vertex = []
            g2d_polar_convex = scans
            g2d_polar_convex_theta = [i*2*np.pi/self.laser_num_beams for i in range(self.laser_num_beams)]
            for i in range(self.laser_num_beams):
                g2d_convex_vertex.append((scans[i]*np.cos(g2d_polar_convex[i]),scans[i]*np.sin(g2d_polar_convex_theta[i])))

 
        if g2d_cal_success and last_polar_action_info is not None:
            s = [npdtheta,nprho,nrvx,nrvy,1.]
        else:
            if g2d_cal_success:
                s = [ndtheta,nrho,nrvx,nrvy,1.]
            else:
                s = [ndtheta,nrho,nrvx,nrvy,0.]


        s.extend(n_polar_action_rad)
        s.extend(n_polar_action_dist)
        s.extend(n_opt_polar_action_rad)
        s.extend(n_opt_polar_action_dist)

        if self.last_done:
            for i in range(self.stack_times):
                self.scan_que[i,:] = process_scan
                self.state_que[i,:] = np.array(s)
        else:
            self.scan_que = np.roll(self.scan_que, -1, axis=0)
            self.scan_que[-1, :] = process_scan
            self.state_que = np.roll(self.state_que, -1, axis=0)
            self.state_que[-1, :] = np.array(s)
    ...
    def get_observations(self, last_polar_action_info,last_world_action_info):
        # apply action time horizon
        # if self._is_train_mode:
        #     # 向前仿真到需要采取下个动作的周期
        #     self.call_service_takeSimStep(self._action_frequency)
        ...
        n_scan,n_polar_convex,ndtheta,nrho,npdtheta,nprho,nrvx,nrvy\
            ,n_polar_action_rad,n_polar_action_dist,n_opt_polar_action_rad,n_opt_polar_action_dist \
            = self.normalize_obs(g2d_cal_success,scans,(g2d_polar_convex_theta,g2d_polar_convex),dtheta,rho,pdtheta,prho,self._robot_vel.linear.x,self._robot_vel.linear.y,last_polar_action_info)
        ...
        #     g2d_polar_convex_theta = [i*2*np.pi/self.laser_num_beams for i in range(self.laser_num_beams)]
        #     for i in range(self.laser_num_beams):
        #         g2d_convex_vertex.append((scans[i]*np.cos(g2d_polar_convex_theta[i]),scans[i]*np.sin(g2d_polar_convex_theta[i])))
        ...
        if g2d_cal_success and len(g2d_polar_convex) == self.max_vertex_num:
            ...
            for i in range(self.max_vertex_num):
                ...
                process_scan.append((scans[di*i]+self.safe_dist)/self.lidar_range)
        ...
        # print(g2d_cal_success)
        # print(g2d_convex_vertex[0],g2d_convex_vertex[-2],g2d_convex_vertex[-1])
        # print(g2d_polar_convex_theta[0],g2d_polar_convex_theta[-2],g2d_polar_convex_theta[-1])
        # print(g2d_polar_convex_theta)
    ...
    def feasible_position(self,scans,g2d_cal_success,g2d_polar_convex):
        ...
        jerk = []
        jerk_max = self.plan_dynamic_limit[2]*1.41421356
    ...
    def normalize_obs(self,g2d_cal_success,scan,polar_convex,theta,rho,pdtheta,prho,wvx,wvy,last_polar_action_and_opt_action):
        # 全部归一化到0-1
        ...
        g2d_polar_convex_theta = polar_convex[0]
        g2d_polar_convex = polar_convex[1]
        if g2d_cal_success and len(g2d_polar_convex) == self.max_vertex_num:
            for i in range(len(g2d_polar_convex)):
                scan_i = (g2d_polar_convex[i] - scan_min)/(scan_max - scan_min)
                if scan_i > 1.:
                    scan_i = 1.
                if scan_i < 0.:
                    scan_i = 0.
                n_polar_convex.append(scan_i)
                n_polar_convex.append(g2d_polar_convex_theta[i]/2./np.pi)
        ...
        if prho is not None:
            nprho = prho/40.
        npdtheta = None
        if pdtheta is not None:
            npdtheta = pdtheta/np.pi
        ...
        rvx,rvy = ObservationCollector_d86.worldva2robotva(self._robot_vel.linear.x,self._robot_vel.linear.y,self._robot_pose.theta)
        nrvx = rvx / self.plan_dynamic_limit[0]
        nrvy = rvy / self.plan_dynamic_limit[0]
        
        n_polar_action_rad,n_opt_polar_action_rad,n_polar_action_dist,n_opt_polar_action_dist = [],[],[],[]
        if last_polar_action_and_opt_action is None:
            for i in range(self.opt_ctrl_points_num):
                n_polar_action_rad.append(0./np.pi)
                n_opt_polar_action_rad.append(0./np.pi)
                n_polar_action_dist.append((0. - scan_min)/(scan_max - scan_min))
                n_opt_polar_action_dist.append((0. - scan_min)/(scan_max - scan_min))
        else:
            polar_action,opt_polar_action = last_polar_action_and_opt_action
            for i in range(self.opt_ctrl_points_num):
                n_polar_action_rad.append(NormalizeAngle(polar_action[i][0])/np.pi)
                n_opt_polar_action_rad.append(NormalizeAngle(opt_polar_action[i][0])/np.pi)
                n_polar_action_dist.append((polar_action[i][1] - scan_min)/(scan_max - scan_min))
                n_opt_polar_action_dist.append((opt_polar_action[i][1] - scan_min)/(scan_max - scan_min))

        return np.array(n_scan),np.array(n_polar_convex),ntheta,nrho,npdtheta,nprho,nrvx,nrvy,n_polar_action_rad,n_polar_action_dist,n_opt_polar_action_rad,n_opt_polar_action_dist
    ...
    def _get_goal_pose_in_robot_frame(self,goal_pos: Pose2D, robot_pos: Pose2D,robot_vel:Twist,last_world_action_info):
        ...
        # print("vtheta")
        # print(v_heading,np.arctan2(y_relative, x_relative),dtheta)


        prho = None
        pdtheta = None
        # if last_polar_action_and_opt_action is not None:
        #     # last_action_point_in_robot[0]为网络输出 極座標下表達 theta,distance
        #     # last_action_point_in_robot[1]为mpc输出
        #     last_p = (last_polar_action_and_opt_action[1][-1][1]*np.cos(last_polar_action_and_opt_action[1][-1][0]),
        #             last_polar_action_and_opt_action[1][-1][1]*np.sin(last_polar_action_and_opt_action[1][-1][0]))
        #     if len(last_polar_action_and_opt_action[1]) == 1:
        #         last_p_1 = (0.,0.)
        #     else:
        #         last_p_1 = (last_polar_action_and_opt_action[1][-2][1]*np.cos(last_polar_action_and_opt_action[1][-2][0]),
        #             last_polar_action_and_opt_action[1][-2][1]*np.sin(last_polar_action_and_opt_action[1][-2][0]))
        #     last_p_w = ObservationCollector_d86.robotp2worldp(last_p,(self._last_robot_pose.x,self._last_robot_pose.y,self._last_robot_pose.theta))
        #     last_p_w_1 = ObservationCollector_d86.robotp2worldp(last_p_1,(self._last_robot_pose.x,self._last_robot_pose.y,self._last_robot_pose.theta))
            
        #     y_relative = goal_pos.y - last_p_w_1[1]
        #     x_relative = goal_pos.x - last_p_w_1[0]
        ...
        if last_world_action_info is not None:
            # print("last mpc world first and last")
            # print(last_world_action_info[-2],last_world_action_info[-1])
            # print("robot and goal pose")
            # print((robot_pos.x,robot_pos.y),(goal_pos.x,goal_pos.y))

            last_p_w = last_world_action_info[-1]
            last_p_w_1 = last_world_action_info[-2]
            y_relative = goal_pos.y - last_p_w_1[1]
            x_relative = goal_pos.x - last_p_w_1[0]
            # prho = np.hypot(goal_pos.x - last_p_w[0],goal_pos.y - last_p_w[1])
            prho = rho
            v_heading = np.arctan2(last_p_w[1] - last_p_w_1[1],last_p_w[0] - last_p_w_1[0])
            pdtheta = NormalizeAngle(v_heading - np.arctan2(y_relative, x_relative))
            # print((x_relative,y_relative),(last_p_w[0] - last_p_w_1[0],last_p_w[1] - last_p_w_1[1]))
            # print(v_heading,np.arctan2(y_relative, x_relative),pdtheta)
    ...
    def callback_galaxy2d(self,msg_galaxy2d):
        self.msg_galaxy2d = msg_galaxy2d
        # self.g2d_cal_success = msg_galaxy2d.success.data
        # if msg_galaxy2d.scans is None or len(msg_galaxy2d.scans) != self.laser_num_beams:
        #     self.scans = [0. for i in range(self.laser_num_beams)]
        # else:
        #     self.scans = msg_galaxy2d.scans

        # self.g2d_convex_vertex = []
        # for p in msg_galaxy2d.convex_vertex.points:
        #     self.g2d_convex_vertex.append((p.x,p.y))

        # self.g2d_polar_convex = msg_galaxy2d.polar_convex
        # self.g2d_polar_convex_theta = msg_galaxy2d.polar_convex_theta
