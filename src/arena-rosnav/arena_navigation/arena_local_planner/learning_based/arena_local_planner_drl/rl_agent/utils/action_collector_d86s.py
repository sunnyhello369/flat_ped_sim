...
class ActionCollector_d86:
    ...
    plan_dynamic_limit=(2.5,4.,8.)):
        ...
        self._action_period = 1. / rospy.get_param("/robot_action_rate") # 控制指令发送到环境的周期1/10=0.1
        self._plan_period = 1. / rospy.get_param("/robot_plan_rate") # 规划周期 1/10=0.1
        self.traj_time_span = rospy.get_param("/traj_time_span") # 运动学模型预测的轨迹时长 eval:3 train:1

        self.output_points_num = output_points_num # 2
        ...
        self.controller = SO2MPC_dense2(        
            ...
            nb_timesteps = int(self.traj_time_span/dt), # eval:3/0.1=30
            smooth_j_weight = 1.,
            # end_weight =  15000000.,
        ...
        # 提供给MPC的整条轨迹的参考状态信息,包含所有时刻的位置速度加速度等 (self.traj_time_span/dt,6)
        # [px,py,vx,vy,ax,ay]
        self.ref = []
        ...
        self.ref[-4] = 0.
        self.ref = np.array(self.ref).T # transposition


        self.mpc_traj_world = None # 列表,包含MPC规划得到的世界坐标系下的轨迹状态,包括位置、速度、加速度等,每个时刻为一个向量。
        self.mpc_traj_robot = None # 布尔类型,表示下一次需要停止不动作。
        ...
        self.new_state_come = False # 布尔类型,表示是否有新的状态信息到达。
    ...
    def record(self):
        if True:
            odom_msg = deepcopy(self.msg_Odometry)
            action_odom = Odometry()
            action_odom.header.stamp = rospy.Time.now()
            action_odom.header.frame_id = 'map'
            # action_odom.header.seq = self.steps
            action_odom.pose.pose.position.z = self.steps
            
            action_odom.pose.covariance[0] = self.start_state_world[0][0]
            action_odom.pose.covariance[1] = self.start_state_world[0][1]
            action_odom.pose.covariance[2] = self.goal_pos_world[0]
            action_odom.pose.covariance[3] = self.goal_pos_world[1]

            if odom_msg is not None:
                action_odom.pose.pose.position.x = odom_msg.pose.pose.position.x
                action_odom.pose.pose.position.y = odom_msg.pose.pose.position.y
                action_odom.twist.twist.angular = odom_msg.twist.twist.angular
                action_odom.twist.twist.linear = odom_msg.twist.twist.linear

            self.action_odom_pub.publish(action_odom)
    ...
    def plan_step(self,action_points,laser_convex,convex_success,robot_pose):
        ...
        # 在安全凸空间内部采样短期和长期的参考位置点，用于指导 MPC 预测的第一个和最后一个控制周期状态的位置
        ...
        if convex_success:
            ...
            # 如果终点在多边形内,将终点设置为全局目标点
            # first solve with convex
            if ActionCollector_d86.is_in_convex(goal_in_robot_frame,laser_convex):
                action_points.pop()
                action_points.append(goal_in_robot_frame)
                # 三阶
        ...
        if not success:
            ...
            if self.mpc_traj_world is not None and self.exc_time + self._plan_period < self.traj_time_span:
                ...
                # second solve without convex
                if self.controller.solve(start_state_robot,self.ref):
                    success = True

                # econd solve fail -> bark
                if not success:
                    if VIS:
                        if PRINT:
                            print("plan convex stop trajfail")
    ...
    def control_exce_multi_step(self):
        ...
        # 在本帧规划计算中，跟踪的是上帧轨迹
        # exce_time_thisloop表示这一规划周期内需要发布的控制指令个数
        exce_time_thisloop = int(self._plan_period/self._action_period)

        for i in range(exce_time_thisloop):
            # 没有考虑位置误差的直接跟踪
            # 将轨迹转到自车坐标系下
            e_index = int((self.exc_time+i*self._action_period + 1e-6)/self._action_period) # 根据当前已执行时间确定在轨迹上的索引
            wax = self.mpc_traj_world[2][e_index][0]
            way = self.mpc_traj_world[2][e_index][1]
            wvx = self.mpc_traj_world[1][e_index][0]
            wvy = self.mpc_traj_world[1][e_index][1]
