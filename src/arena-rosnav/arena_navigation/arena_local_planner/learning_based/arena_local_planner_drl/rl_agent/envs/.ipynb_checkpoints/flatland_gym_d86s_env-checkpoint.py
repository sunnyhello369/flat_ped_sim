...
class FlatlandEnv_d86(gym.Env):
    ...
    def step(self, action: np.ndarray):
        ...
        if self.last_obs_dict["laser_convex"][0] is not None:
            ...
            one_action_point1,one_polar_action1 = self.calc_polar_action_points((0.,0.,np.arctan2(rvy,rvx)),(action[0],action[1]))
    ...
    def reset(self):
        ...
        while True:
            ...
            # print(self.ns + "re8")
            if self.last_obs_dict["laser_scan"].min() > 2.5:
                break
    ...
    def calc_polar_action_points(self,start,action):
        theta = NormalizeAngleTo2Pi(2*np.pi*action[0] + start[2])
        min_index = 0
        min_theta = 3*np.pi
        thetas = []
        for i in range(self.observation_collector.max_vertex_num):
            p = (self.last_obs_dict["laser_convex"][1][i] * np.cos(self.last_obs_dict["laser_convex"][2][i]), \
                self.last_obs_dict["laser_convex"][1][i] * np.sin(self.last_obs_dict["laser_convex"][2][i]))
        ...
        crossp = self.calc_cross_point(start, \
                                ...
                                (self.last_obs_dict["laser_convex"][1][prev] * np.cos(
                                    self.last_obs_dict["laser_convex"][2][prev]), \
                                self.last_obs_dict["laser_convex"][1][prev] * np.sin(
                                    self.last_obs_dict["laser_convex"][2][prev])), \
                                (self.last_obs_dict["laser_convex"][1][next] * np.cos(
                                    self.last_obs_dict["laser_convex"][2][next]), \
                                self.last_obs_dict["laser_convex"][1][next] * np.sin(
                                    self.last_obs_dict["laser_convex"][2][next])))
