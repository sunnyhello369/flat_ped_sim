...
def get_parent_directory(target_string):
    ...
    if target_string in path_parts:
        ...
        return None
area_path = get_parent_directory("flat_ped_sim") + "src/arena-tools"

sys.path.append(area_path)
    ...
    def reset(self):
        ...
        with self._map_lock:
            ...
            try:
                ...
                # self.dynamic_obs_xy_list,self.static_obs_xy_list = self.obstacles_manager.reset_pos_obstacles_random(
                self.obstacles_manager.reset_pos_obstacles_random(
                    ...
                    forbidden_zones=[
                        (
                            start_pos.x,
                            start_pos.y,
                            self.robot_manager.ROBOT_RADIUS+1.5,
                        ),
                        (
                            goal_pos.x,
                            goal_pos.y,
                ...
                self.start_pos_task = start_pos
                self.goal_pos_task = goal_pos
            ...
            if fail_times >= max_fail_times:
                print(self.robot_manager.ns+"task reset fail")
                rospy.logwarn(self.robot_manager.ns+"task reset fail!")
...
class ScenerioTask(ABSTask):
    ...
    def reset(self):
        ...
        with self._map_lock:
            ...
            start_pos = Pose2D()
            start_pos.x = robot_start_pos[0]
            start_pos.y = robot_start_pos[1]
            goal_pos = Pose2D()
            goal_pos.x = robot_goal_pos[0]
            goal_pos.y = robot_goal_pos[1]
...
class ScenarioTask(ABSTask):
    def __init__(
        ...
        obstacles_manager: ObstaclesManager,
        robot_manager: RobotManager,
        scenario_path: str,ScenarioTask
    ):
        super().__init__(obstacles_manager, robot_manager)
    ...
    def reset(self):
        ...
        with self._map_lock:
            ...
            start_pos = Pose2D()
            start_pos.x = self.scenario.robotPosition[0]
            start_pos.y = self.scenario.robotPosition[1]
            goal_pos = Pose2D()
            goal_pos.x = self.scenario.robotGoal[0]
            goal_pos.y = self.scenario.robotGoal[1]
...
def get_predefined_task(ns: str, mode="random", start_stage: int = 1, PATHS: dict = None):
    ...
    obstacles_manager = ObstaclesManager(ns, map_response.map,robot_manager.ROBOT_RADIUS)
